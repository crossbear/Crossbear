...
wget -q http://archive.routeviews.org/bgpdata/`date +%Y.%m`/RIBS/rib.`date +%Y%m%d`.2000.bz2 --output-document=$3curr_rib_dump.bz2
cd pyasn-read-only/converter/
python convert_rib.py $3curr_rib_dump.bz2 $1
